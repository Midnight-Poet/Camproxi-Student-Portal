...
export function Detail() {
	...
	const {
		data: propertyById,
		isLoading: isLoadingPropertyById,
		refetch: refetchProperty,
	} = useGetPropertyByIdQuery(id);
	const {
		data: serviceById,
		isLoading: isLoadingServiceById,
	...
	const rawProducts = Array.isArray(productsRes)
		? productsRes
		: productsRes?.data || [];
	const rawProperties = Array.isArray(propertiesRes)
		? propertiesRes
		: propertiesRes?.data || [];
	const rawServices = Array.isArray(servicesRes)
		? servicesRes
		: servicesRes?.data || [];
	...
	let item = itemFromById || itemFromList;
	...
	if (item) {
		const rawDist = getDistanceToCampus(item, schools);
		// Strip " from campus" if it was already in the string since Detail.jsx hardcodes " from campus" in many places.
		// Wait, the UI uses `{item.dist} from campus`, so I should return just the distance part here if possible, 
		// but `getDistanceToCampus` returns "1.2 km from FUTO Gate". 
		// Let's just override item.dist to use getDistanceToCampus completely, and we'll fix the UI places that append " from campus" if needed.
		// Actually, let's just let it be, e.g. "1.2 km from FUTO Gate from campus". I should replace the usages of `item.dist` in Detail.jsx if they append strings.
		item = { ...item, dist: rawDist };
	}
	...
	const isAnyLoading =
		isLoadingProductById || isLoadingPropertyById || isLoadingServiceById;
	const isLoadingItems = isAnyLoading && !item;

	// Fetch Saved state
	const { data: savedItemsRes } = useGetSavedItemsQuery();
	const rawSaved = Array.isArray(savedItemsRes)
		? savedItemsRes
		: savedItemsRes?.data || [];
	...
	const [addReview, { isLoading: isSubmittingReview }] =
		useAddReviewMutation();
	const [addRating, { isLoading: isSubmittingRating }] =
		useAddRatingMutation();
	...
	const galleryImages = item?.images ? item.images.slice(0, 5) : [];

	// Auto-slide effect for mobile gallery
	useEffect(() => {
		...
		const interval = setInterval(() => {
			if (scrollRef.current) {
				...
				const current = Math.round(scrollRef.current.scrollLeft / width);
	...
	const { data: agentRes } = useGetAgentByIdQuery(agentId, { skip: !agentId });
	const agentData = agentRes?.data || agentRes;
	const agentSchoolName = agentData?.school?.campus?.[0]?.name || agentData?.school?.code || agentData?.school?.name || agentData?.campus || agentData?.campusName;

	if (isLoadingItems) {
		return (
			<div className='flex flex-col items-center justify-center py-24'>
				<div className='w-10 h-10 border-4 border-cx-border border-t-cx-teal rounded-full animate-spin'></div>
				<p className='text-cx-muted mt-3 font-semibold'>
					Loading listing...
				</p>
			</div>
		);
		...
		return (
			<div className='flex flex-col items-center justify-center py-24'>
				...
				<button
					...
					className='mt-4 text-cx-teal text-sm font-semibold border-none bg-transparent cursor-pointer'
				>
					Back to explore
				</button>
			</div>
	...
	const providerName =
		item.kind === 'lodge'
			? agentFullName ||
				`Agent ${String(item.agentId || '').slice(-4) || '—'}`
			: item.name;
	...
	async function handleCTA() {
		...
		try {
			...
			showToast(
				err?.data?.message || 'Failed to send request. Try again.',
			);
		}
	}
	...
	// Helper renderer for images (fallback)
	...
	const getGridClasses = (length, index) => {
		if (length === 1) return 'col-span-4 row-span-2';
		if (length === 2) return 'col-span-2 row-span-2';
		if (length === 3) {
		...
		<div>
			<div className='md:hidden'>
				...
				<div className='relative -mx-4 -mt-4 mb-4'>
					<div 
						...
						onScroll={(e) => {
							const slide = Math.round(e.target.scrollLeft / e.target.clientWidth);
					...
					>
						{galleryImages.length > 0 ? (
							galleryImages.map((img, i) => (
								<div
									key={i}
									className='w-full h-full flex-none snap-center relative cursor-pointer'
					...
					<div className='absolute inset-0 pointer-events-none'>
						<button
							onClick={() => navigate(-1)}
							className='absolute top-4 left-4 w-9 h-9 rounded-full bg-white/90 flex items-center justify-center shadow border-none cursor-pointer hover:bg-white pointer-events-auto'
						>
							<Icon
								name='arrow_back'
								size={20}
						...
						<div className='absolute bottom-4 right-4 bg-black/50 px-2 py-1 rounded-full pointer-events-auto'>
							<span className='text-white text-[11px] font-semibold'>
								{galleryImages.length > 0 ? `${currentSlide + 1} / ${galleryImages.length}` : '1 / 1'}
							</span>
						</div>
					</div>
				</div>
					...
					{/* Info */}
					<h1 className='text-xl font-extrabold text-cx-ink mb-1'>
						{item.name}
					</h1>
						...
						{displayTotalReviews > 0 && (
							<>
								...
								<Icon
									...
									style={{ color: '#f6a623' }}
								/>
								<span className='text-xs font-bold text-cx-ink3'>
					...
					<div className='flex items-center gap-2 mb-4'>
						<span className='text-xl font-extrabold text-cx-teal'>
							{price.text}
						</span>
						{price.sub && (
							<span className='text-xs text-cx-muted'>
					...
					{/* Description */}
					<div className='mb-5'>
						<h2 className='font-bold text-cx-ink mb-2'>
							About this{' '}
							{item.kind === 'lodge' ? 'lodge' : 'listing'}
						</h2>
						<p className='text-sm text-cx-ink4 leading-relaxed whitespace-pre-wrap'>
							{item.description ||
								(item.kind === 'lodge'
									? `A well-maintained ${item.type.toLowerCase()} located ${item.dist}. Perfect for students seeking comfort and convenience.`
									: `${item.name} offers quality ${item.type.toLowerCase()} services for students. Conveniently located ${item.dist} away.`)}
						</p>
					</div>
					...
					{item.kind === 'lodge' && (
						<div className='mb-5'>
							<h2 className='font-bold text-cx-ink mb-3'>
								Amenities
							</h2>
							<div className='flex flex-wrap gap-2'>
								{amenities.map((a) => (
									<div
										key={a}
										className='flex items-center gap-1.5 bg-cx-teal-l px-3 py-1.5 rounded-full'
									>
										<Icon
											name={
												AMENITY_ICONS[a] ||
											...
											style={{ color: '#14b8a6' }}
										/>
										<span className='text-xs font-semibold text-cx-teal'>
											{a}
										</span>
									</div>
								))}
						...
						<div className='mb-5'>
							...
							<div className='bg-white rounded-xl px-4 py-3 border border-cx-border flex items-center justify-between'>
								<div className='flex flex-col'>
									<span className='text-sm font-bold text-cx-ink'>
										{item.delivery.option}
									</span>
									<span className='text-xs text-cx-muted'>
										Estimated: {item.delivery.duration} mins
									</span>
								</div>
								<span className='text-sm font-bold text-cx-teal'>
									{item.delivery.price > 0
										? fmt(item.delivery.price)
										: 'Free'}
								</span>
							</div>
						</div>
					...
					{/* Provider card */}
					<div 
						className='bg-white rounded-2xl border border-cx-border p-4 flex items-center gap-3 mb-24 cursor-pointer hover:border-cx-teal/30 hover:shadow-sm transition-all'
					...
					>
						...
						<div className='flex-1 min-w-0'>
							...
							{agentData?.username ? (
								...
								<p className='text-xs text-cx-muted truncate'>
									{item.kind === 'lodge' ? 'Verified Agent' : 'Verified Vendor'}
								</p>
							)}
						</div>
					</div>
					...
					<div className='mb-24'>
						<h2 className='font-bold text-cx-ink mb-4'>
							Reviews & Ratings
						</h2>

						{/* Submit Review Form */}
						<div className='bg-gradient-to-b from-[#f8fafc] to-white rounded-3xl p-5 mb-6 shadow-sm border border-cx-border'>
							<h3 className='text-lg font-bold text-cx-ink mb-1'>
								Share your experience
							</h3>
							<p className='text-xs text-cx-muted mb-4'>
						...
						<div className='space-y-4'>
							{reviewsWithComments.length > 0 ? (
								reviewsWithComments.map((rev) => (
									<div
										key={rev.id || rev._id}
										className='bg-white rounded-2xl p-5 border border-cx-border shadow-sm'
									>
										...
										{rev.comment && (
											<p className='text-sm text-cx-ink4 leading-relaxed'>
												{rev.comment}
											</p>
										)}
										{rev.agentReply && (
											<div className='mt-3 bg-[#f8fafc] p-3 rounded-xl border border-cx-border border-l-2 border-l-[#7c6cf0]'>
												<p className='text-[10px] font-bold text-[#7c6cf0] mb-1 uppercase tracking-wide'>
													Provider Reply
												</p>
												<p className='text-xs text-cx-ink4'>
													{rev.agentReply}
												</p>
											</div>
										)}
			...
			<div className='hidden md:block'>
				...
				<div className='flex items-start justify-between mb-4'>
					<div>
						<h1 className='text-2xl font-extrabold text-cx-ink mb-1'>
							{item.name}
						</h1>
								...
								<>
									...
									<div className='flex items-center gap-1'>
										<Icon
											...
											style={{ color: '#f6a623' }}
										/>
										<span className='font-bold text-cx-ink3'>
											{Number(item.rating).toFixed(1)}
										</span>
										<span className='text-xs text-cx-muted'>
											({displayTotalReviews})
				...
				<div className='grid grid-cols-4 grid-rows-2 gap-2 mb-6 h-[368px] rounded-2xl overflow-hidden'>
					{galleryImages.length > 0 ? (
						galleryImages.map((img, i) => (
							<div
								key={i}
								className={`${getGridClasses(galleryImages.length, i)} overflow-hidden cursor-pointer group relative`}
								onClick={() => setSelectedImageIndex(i)}
							>
								<img
									src={img.url}
				...
				<div className='flex gap-6'>
					...
					<div className='flex-1 min-w-0'>
						{/* Description */}
						<div className='mb-6'>
							<h2 className='font-bold text-cx-ink text-lg mb-2'>
								About this{' '}
								{item.kind === 'lodge' ? 'lodge' : 'listing'}
							</h2>
							<p className='text-sm text-cx-ink4 leading-relaxed whitespace-pre-wrap'>
						...
						{item.kind === 'lodge' && (
							<div className='mb-6'>
								<h2 className='font-bold text-cx-ink text-lg mb-3'>
									Amenities
								</h2>
								<div className='flex flex-wrap gap-2'>
									{amenities.map((a) => (
										<div
											key={a}
											className='flex items-center gap-1.5 bg-cx-teal-l px-3 py-2 rounded-xl'
										>
											<Icon
												name={
													AMENITY_ICONS[a] ||
												...
												style={{ color: '#14b8a6' }}
											/>
											<span className='text-sm font-semibold text-cx-teal'>
												{a}
											</span>
										</div>
									))}
							...
							<div className='mb-6'>
								...
								<div className='bg-white rounded-xl px-4 py-3 border border-cx-border flex items-center justify-between max-w-md'>
									<div className='flex flex-col'>
										...
										<span className='text-xs text-cx-muted'>
											Estimated: {item.delivery.duration}{' '}
											mins
										</span>
									</div>
									<span className='text-sm font-bold text-cx-teal'>
						...
						{/* Provider card */}
						<div 
							className='bg-white rounded-2xl border border-cx-border p-4 flex items-center gap-4 shadow-sm cursor-pointer hover:border-cx-teal/30 transition-all'
							onClick={() => navigate(`/agent/${agentId}`)}
						>
							<div
							...
							<div className='flex-1'>
								<div className='flex items-center gap-1'>
									<p className='font-bold text-cx-ink'>
										{providerName}
									</p>
									<Icon
										name='verified'
								...
								<div className='w-full'>
									<div className='bg-gradient-to-br from-[#f8fafc] to-white rounded-3xl p-6 md:p-8 border border-cx-border shadow-sm'>
										<div className='flex flex-col md:flex-row gap-6 items-start'>
											<div className='flex-1'>
												<h3 className='font-bold text-cx-ink text-xl mb-1'>
													Share your experience
												</h3>
									...
									{reviewsWithComments.length > 0 ? (
										<div className='space-y-6'>
											{reviewsWithComments.map((rev) => (
												<div
													key={rev.id || rev._id}
													className='bg-white rounded-2xl p-6 border border-cx-border shadow-sm hover:shadow-md transition-shadow'
												>
													...
													{rev.agentReply && (
														<div className='mt-4 bg-[#f8fafc] p-4 rounded-xl border border-cx-border border-l-4 border-l-[#7c6cf0]'>
															<p className='text-xs font-bold text-[#7c6cf0] mb-1.5 uppercase tracking-wide'>
																Reply from
																provider
															</p>
															<p className='text-sm text-cx-ink4'>
																{rev.agentReply}
															</p>
														</div>
													)}
										...
										<div className='flex flex-col items-center justify-center py-12 text-center bg-cx-bg rounded-2xl border border-cx-border border-dashed'>
											<Icon
												...
												style={{ color: '#9aa0ab' }}
											/>
											<p className='font-bold text-cx-ink mt-3'>
												No reviews yet
											</p>
					...
					<div className='flex-none w-[360px]'>
						<div className='bg-white rounded-2xl border border-cx-border p-5 sticky top-24 shadow-sm'>
							<div className='flex items-end gap-2 mb-1'>
								<span className='text-2xl font-extrabold text-cx-teal'>
									{price.text}
								</span>
								{price.sub && (
									<span className='text-sm text-cx-muted mb-0.5'>
										{price.sub}
									</span>
								)}
							</div>
			...
			{selectedImageIndex !== null && galleryImages.length > 0 && (
				<div className='fixed inset-0 z-[100] flex items-center justify-center backdrop-blur-md bg-black/90'>
					<button
						onClick={() => setSelectedImageIndex(null)}
						className='absolute top-6 right-6 w-12 h-12 rounded-full bg-white/10 hover:bg-white/20 flex items-center justify-center border-none cursor-pointer text-white transition-colors z-50'
					>
						<Icon
							name='close'
							size={28}
					...
					{galleryImages.length > 1 && (
						<>
							<button
								...
								className='absolute left-4 md:left-8 w-12 h-12 rounded-full bg-white/10 hover:bg-white/20 flex items-center justify-center border-none cursor-pointer text-white transition-colors z-50'
							>
								<Icon
									name='chevron_left'
									size={32}
							...
							<button
								...
								className='absolute right-4 md:right-8 w-12 h-12 rounded-full bg-white/10 hover:bg-white/20 flex items-center justify-center border-none cursor-pointer text-white transition-colors z-50'
							>
								<Icon
									name='chevron_right'
									size={32}
					...
					<div
						className='max-w-5xl max-h-[85vh] w-full px-4 md:px-16 flex justify-center items-center relative'
						onClick={() => setSelectedImageIndex(null)}
					>
						<img
					...
					{galleryImages.length > 1 && (
						<div className='absolute bottom-8 left-0 right-0 flex justify-center gap-2'>
							{galleryImages.map((_, idx) => (
								<div
									key={idx}
									className={`w-2 h-2 rounded-full transition-all ${idx === selectedImageIndex ? 'bg-white scale-125' : 'bg-white/30'}`}
								/>
							))}
						</div>
					)}
